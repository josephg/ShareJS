...
Session.prototype._subscribeToStream = function(collection, doc, opstream) {
  var value = this._isSubscribed(collection, doc);
  if (!value) {
...
// {c:, doc:, v:, type:, snapshot:}. So I have to rewrite them.
Session.prototype._processQueryResults = function(results, qopts, callback) {
  var session = this;
  var pending = 1;
  console.log('pqr');
  var done = function() {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    var collection = r.c;
    var doc = r.doc = r.docName;
    //delete r.docName;

    if (qopts.docMode && !this._isSubscribed(collection, doc)) {
      var atVersion = qopts.versions[collection] && qopts.versions[collection][doc];
      ...
      if (atVersion == null) {
        ...
        (function(collection, doc) {
          session.agent.getOps(collection, doc, atVersion, -1, function(err, results) {
            if (err) {
              session._send({a:'fetch', c:collection, doc:doc, error:err});
              return done();
            }
      ...
      if (qopts.docMode === 'sub') {
        ...
        pending++;
        console.log('pending subscribe to ' + doc + ' pending: ', pending);

        (function(collection, doc) {
          session.agent.subscribe(collection, doc, r.v, function(err, opstream) {
            if (err) {
              session._setSubscribed(collection, doc, false);
              session._send({a:'sub', c:collection, doc:doc, error:err});
              return;
            }
  ...
  // Don't call the callback until the next tick, at the earliest. This is so
  // the query handling code below can reply to the query before any other
  // messages get sent to the client.
  ...
  switch (req.a) {
    ...
    case 'qfetch':
      ...
      agent.queryFetch(index, req.q, qopts, function(err, results, extra) {
        ...
        session._processQueryResults(results, qopts);
    ...
    case 'qsub':
      ...
      agent.query(index, req.q, qopts, function(err, emitter) {
        ...
        session._processQueryResults(emitter.data, qopts);
        ...
        emitter.on('diff', function(diff) {
          for (var i = 0; i < diff.length; i++) {
            var d = diff[i];
            if (d.type === 'insert') {
              session._processQueryResults(d.values, qopts);
            }
          }
