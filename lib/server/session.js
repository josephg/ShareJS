...
//
// The wire protocol is specced out here, but not updated for the 0.7 changes:
// https://github.com/josephg/ShareJS/wiki/Wire-Protocol

var hat = require('hat');

// Create and return a new session.
module.exports = function(backend, stream, initialReq) {
  var session = new Session(backend, stream);
  session.start(initialReq);
  return session;
};
 ...
 *
 * @param {ShareServer} instance
 * @param {Duplex} stream connection to a client
 * @param {initialReq} optional initial http request which initiated the
 *        connection.
 */
function Session(backend, stream) {
  ...
  this.agent = backend.as ? backend.as(this) : backend;
...
// Actually start talking to the client. It might make more sense to call this
// at the end of the constructor. initialReq is optional.
// 
// This function could take a callback argument but I can't think of any decent
// use cases for it to do so.
Session.prototype.start = function(initialReq) {
  // Initialize the remote client by sending it its session Id. It makes more
  // logical sense to put this in the agent.connect() callback, but doing so
  // would cause an extra browserchannel roundtrip, and I don't think it buys us
  // anything.
  this._send({a:'init', protocol:1, id:this.sessionId});
  
  var self = this;

  if (this.agent.connect) {
    this.agent.connect(this.stream, initialReq, function(err) {
      if (err) return self.close(err);
...
Session.prototype.sendUserData = function(data) {
  this._send({a:'usr', d:msg});
};
...
Session.prototype._handleMessage = function(req, callback) {
  ...
  if ((error = this._checkRequest(req))) {
    console.warn('Warning: Invalid request from ', this.sessionId, req, 'Error: ', error);
  ...
  if (req.a === 'qsub' || req.a === 'qfetch' || req.a === 'qunsub') {
    ...
    if (req.d === null) {
      ...
      if (!this.lastReceivedDoc || !this.lastReceivedCollection) {
        console.warn("msg.d or collection missing in req " + JSON.stringify(req) + " from " + this.sessionId);
  ...
  switch (req.a) {
    ...
    case 'usr':
      agent.onUserData && agent.onUserData(msg.d);
      break;
