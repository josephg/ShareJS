  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (qopts.docMode && !this._isSubscribed(collection, doc)) {
      ...
      if (atVersion == null) {
        ...
        this.agent.getOps(collection, doc, atVersion, -1, function(err, results) {
          if (err) return; // ?? I guess I could send an error to the client here..?
      ...
      if (qopts.docMode === 'sub') {
        ...
        this.agent.subscribe(collection, doc, r.v, function(err, opstream) {
          if (err) {
            session._setSubscribed(collection, doc, false);
            session._send({a:'sub', error:err});
          }
