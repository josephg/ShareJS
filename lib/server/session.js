...
Session.prototype._subscribeToStream = function(collection, docName, opstream) {
  ...
  opstream.once('end', function() {
    ...
    // Remove the client's session information when the stream closes.
    console.log('remove presence for ' + self.agent.sessionId);
    self.agent.backend.submitPresence(collection, docName, {p:[self.agent.sessionId]});
  });
};
...
Session.prototype._subscribe = function(collection, docName, v, callback) {
  ...
  if (v != null) {
    ...
    this.agent.subscribe(collection, docName, v, {wantPresence:true}, function(err, opstream, presence) {
      if (err) {
        self._setSubscribed(collection, docName, false);
        return callback(err);
      }
    ...
    this.agent.fetchAndSubscribe(collection, docName, function(err, data, opstream, presence) {
      if (err) {
        self._setSubscribed(collection, docName, false);
        return callback(err);
      }

      callback(null, data, presence);
      self._subscribeToStream(collection, docName, opstream);
    });
  }
};
...
Session.prototype.bulkSubscribe = function(request, callback) {
  ...
  agent.bulkFetch(needFetch, function(err, snapshots) {
    ...
    agent.bulkSubscribe(request, options, function(err, streams, presence) {
      if (err) {
        cancel();
        return callback(err);
      }
...
Session.prototype._sendOp = function(collection, docName, data) {
  if (data.pOp && data.pOp.src === this.agent.sessionId) return;
...
Session.prototype._checkRequest = function(req) {
  ...
  } else if (req.a === 'op' || req.a === 'p' || req.a === 'sub' || req.a === 'unsub' || req.a === 'fetch') {
    // Doc-based request.
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (qopts.docMode && !this._isSubscribed(collection, docName)) {
      ...
      if (qopts.docMode === 'sub') {
        ...
        this._setSubscribed(collection, docName, true);
        (function(collection, docName) {
          self.agent.subscribe(collection, docName, atVersion || r.v, {wantPresence:true}, function(err, opstream) {
            if (err) {
              self._setSubscribed(collection, docName, false);
...
Session.prototype._handleMessage = function(req, callback) {
  ...
  switch (req.a) {
    ...
    case 'sub':
      ...
      this._subscribe(collection, docName, req.v, function(err, data, presence) {
        if (err) return callback(err);
    ...
    case 'bs':
      this.bulkSubscribe(req.s, {wantPresence: true}, function(err, response, presence) {
        callback(err, err ? null : {s:response, p: presence});
      });
      break;
    ...
    case 'p':
      // Presence operation. This is usually an update of the selection.
      console.log("Presence", req);
      var path = req.key != null ? [agent.sessionId, req.key] : [agent.sessionId];
      var pOp = {src:agent.sessionId, v:req.v, p:path, val:req.val};
      // var pOp = req.s != null ?
      //   {src:agent.sessionId, v:req.v, p:[agent.sessionId, '_selection'], val:req.s} :
      //   {src:agent.sessionId, v:req.v, p:[agent.sessionId, req.key], val:req.val};

      agent.backend.submitPresence(collection, docName, pOp, function(err) {
        callback(err);
      });
      break;
