...
UserAgent.prototype.bulkFetchCached = function(collection, docNames, callback) {
  var agent = this;

  function asyncMap (list, forEach, allDone) {
    var remaining = list.length;
    var values = [];
    // Handle empty list case
    if (! remaining) return allDone(null, values);

    for (var i = 0, l = list.length; i < l; i++) {
      var eachCb = (function (i) {
        return function(err, value) {
          ...
          remaining--;
          values[i] = value;
          if (! remaining) allDone(null, values);
        };
      })(i);
      if (remaining) forEach(list[i], eachCb, i);
    }
  }

  asyncMap(docNames, function(docName, mapCb) {
    agent.trigger('fetch', collection, docName, function (err, action) {
      mapCb(err);
    });
  }, function(err, actions) {
    if (err) return callback(err);
    agent.backend.bulkFetchCached(collection, docNames, function (err, dataList) {
      if (err) return callback(err);
      if (dataList && dataList.length) {
        asyncMap(dataList, function(data, eachDataCb, dataIndex) {
          var docName = docNames[dataIndex];
