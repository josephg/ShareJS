...
UserAgent.prototype.wrapOpStream = function(collection, docName, stream) {
  // agent has a silly name because it makes it easier to trace memory leaks in
  // this function.
  var agentX = this;
  ...
  function transform(data, encoding, callback) {
    agentX.filterOp(collection, docName, data, afterFilter);

    function afterFilter(err, data) {
 ...
 *   { version: version }
 */
UserAgent.prototype.subscribe = function(collection, docName, version, options, callback) {
  ...
  var agent = this;
  agent.trigger('subscribe', collection, docName, {version:version, options:options}, function(err, action) {
    if (err) return callback(err);
    collection = action.collection;
    docName = action.docName;
    version = action.version;
    options = action.options;
    agent.backend.subscribe(collection, docName, version, options, function(err, stream, presence) {
       callback(err, agent.wrapOpStream(collection, docName, stream), presence);
    });
  });
};

// requests is a map from cName -> docName -> version.
UserAgent.prototype.bulkSubscribe = function(requests, options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  ...
  if (this.instance._hasMiddleware('bulk subscribe') || !this.instance._hasMiddleware('subscribe')) {
    ...
    agent.trigger('bulk subscribe', null, null, {requests:requests, options: options}, function(err, action) {
      if (err) return callback(err);
      requests = action.requests;

      agent.backend.bulkSubscribe(requests, options, function(err, streams, presence) {
        callback(err, err ? null : agent.wrapOpStreams(streams), presence);
      });
    });
  } else {
...
UserAgent.prototype.fetchAndSubscribe = function(collection, docName, callback) {
  ...
  agent.trigger('fetch', collection, docName, function(err, action) {
    ...
    agent.trigger('subscribe', action.collection, action.docName, function(err, action) {
      ...
      collection = action.collection;
      docName = action.docName;
      agent.backend.fetchAndSubscribe(action.collection, action.docName, function(err, data, stream, presence) {
        if (err) return callback(err);
        agent.filterDoc(collection, docName, data, function (err, data) {
          if (err) return callback(err);
          var wrappedStream = agent.wrapOpStream(collection, docName, stream);
          callback(null, data, wrappedStream, presence);
        });
      });
    });
  });
