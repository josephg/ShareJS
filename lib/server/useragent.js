...
UserAgent.prototype.submit = function(collection, docName, opData, callback) {
  ...
  agent.trigger('submit', collection, docName, {opData:opData}, function(err, action) {
    ...
    opData.preValidate = function(opData, oldSnapshot, callback) {
      agent.trigger('pre validate', collection, docName, {opData:opData, oldSnapshot:oldSnapshot}, callback);
    };
    ...
    agent.backend.submit(action.collection, action.docName, action.opData, function(err, v, ops) {
      if (err) callback(err);
      
      agent.trigger('after submit', null, null, action, function() {
        callback(err, v, ops);
      }
    });
  });
