...
function Connection(socket) {
  ...
  // Set to true to console.log all sent & recieved messages.
  this.debug = false;
...
Connection.prototype.bindToSocket = function(socket) {
  ...
  socket.onopen = function(event) {
    connection.emit('socket open', event);
  ...
  socket.onerror = function(event) {
    ...
    connection.emit('socket error', event);
  };

  socket.onclose = function(event) {
    connection.emit('socket close', event);
    connection._setState('disconnected', event.reason || event);
  };
};
...
Connection.prototype.handleMessage = function(msg) {
  ...
  switch (msg.a) {
    case 'init':
      ...
      // ID.
      if (msg.protocol !== 1)
        return this.emit('error', Error('Invalid protocol version'));
...
// automatically when documents are subscribed & send ops. You should never have
...
Connection.prototype.send = function(msg) {
  ...
  if (msg.d) { // The document the message refers to. Not set for queries and user data
...
Connection.prototype.sendUserData = function(msg) {
  this.send({a:'usr', d:msg});
};
