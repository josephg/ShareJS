...
var Doc = exports.Doc = function(connection, collection, name) {
  ...
  // The user's selection (cursor position). This is an opaque structure with
  // data depending on the OT type. For text documents, this is a range
  // ([from, to]).
  this.selection = null;
...
Doc.prototype.flush = function() {
  ...
  } else if (!this.paused && this.selectionDirty) {
    this._send({a:'p', v:this.version, s:this.selection, v:this.version});
  }
};
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  if (opData.op && this.selection != null && this.type.transformSelection) {
    this.selection = this.type.transformSelection(this.selection, opData.op, true);
    this.selectionDirty = false;
  }
