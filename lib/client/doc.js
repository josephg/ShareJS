 ...
 * - created (context): The document was created. That means its type was set and it has
 *   some initial data.
...
// Apply the given op data to our local model.
//
// @private
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  if (previous && tryCompose(this.type, entry, opData)) {
    operation = previous;
  } else {
    operation = opData;
