...
Doc.prototype._onMessage = function(msg) {
  ...
  switch (msg.a) {
    ...
    case 'ack':
      ...
      // resent an op that the server already got. It may also be confirmed
      // normally.
      if (msg.error && msg.error === 'Op already submitted') {
        // The op was already committed successfully. Increment the version number
        this.version++;
    ...
    case 'op':
      ...
      if (msg.v > this.version) {
        ...
        // HACK: get the operations we missed and catch up
        var wantSubscribe = this.wantSubscribe;
        this._setWantSubscribe(false);
        this._setWantSubscribe(wantSubscribe);
        break;
      }
...
Doc.prototype._opAcknowledged = function(msg) {
  ...
  if (!this.state) {
    ...
    if (msg.v !== this.version) {
      throw new Error('Invalid version from server. This can happen when you submit ops in a submitOp callback. Expected: ' + this.version + ' Message version: ' + msg.v);
    }
  }
