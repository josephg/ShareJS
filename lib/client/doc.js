...
Doc.prototype.subscribe = function() {
  if (!this.subscribed) {
    ...
    this.connection.sendSubscribe(this.collection, this.name, this.version);
  }
};
...
Doc.prototype.ingestData = function(data) {
  if (this.version != null) {
    console.warn('Ignoring attempt to ingest data');
    return;
  }
...
Doc.prototype._sendOpData = function() {
  ...
  // The first time we send an op, its id and sequence number are implied by the
  // connection. (This is a microoptimization for the 99% case)
  if (!d.src) {
