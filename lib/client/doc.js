...
Doc.prototype._otApply = function(opData, context) {
  ...
  if (opData.op) {
    ...
    for (var j = 0; j < contexts.length; j++) {
      var kontext = contexts[j];
      if (kontext !== context && kontext._onOp) kontext._onOp(opData.op);
    }
    for (var k = 0; k < contexts.length; k++) {
      if (contexts[k].remove) contexts.splice(k--, 1);
    }
...
Doc.prototype._sendOpData = function() {
  ...
  if (d.src == null) d.src = this.connection.id;
  if (d.seq == null) d.seq = this.connection.seq++;
...
Doc.prototype._tryRollback = function(opData) {
  ...
  if (opData.create) {
    ...
    if (this.state === 'floating') this.state = null;
    else console.warn('Rollback a create from state ' + this.state);
