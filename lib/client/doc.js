...
var Doc = exports.Doc = function(connection, collection, name) {
  ...
  // Presence information for me. This is separate from the presence information
  // because our id will change.
  this.myPresence = {};

  // Mark that we need to send the server our cursor position
  this.myPresenceDirty = false;

  // Map from session ID -> {cursor:..., ....}
  this.peers = {};
};
...
Doc.prototype._setType = function(newType) {
  ...
  // Fix for old ottypes
  if (newType && newType.transformCursor && !newType.transformSelection)
    newType.transformSelection = newType.transformCursor;
...
Doc.prototype.ingestData = function(data) {
  ...
  this.version = data.v;
  // data.data is what the server will actually send.
  this.snapshot = data.data;
  this._setType(data.type);
  this.meta = data.m;
  this.presence = data.p;
  this.state = 'ready';
  this.emit('ready');
  this._onPresenceUpdated();
};
...
Doc.prototype.flush = function() {
  ...
  if (!this.paused && this.pendingData.length && this.connection.state === 'connected') {
    ...
    // _sendOpData also sets action to 'submit'.
    this._sendOpData();
    ...
    this.action = 'fetch';
    // TODO: Currently ignoring this.trackPresence - should set msg.p = false.
    ...
    // TODO: Currently ignoring this.trackPresence - should set msg.p = false somehow.
    this.connection.sendSubscribe(this.collection, this.name, this.state === 'ready' ? this.version : null);
  } else if (!this.paused && this.myPresenceDirty) {
    this._send({a:'p', v:this.version, val:this.myPresence});
    this.myPresenceDirty = false;
  }
};
...
Doc.prototype._otApply = function(opData, context) {
  ...
  _updatePresence(this.type, this.presence, opData);
  ...
  if (opData.op) {
    ...
    this.emit('after op', opData.op, context);
  }

  this._onPresenceUpdated();
  // console.log("new presence -> ", this.presence);
};


Doc.prototype._onPresenceUpdated = function(context) {
  this.emit('presence', this.presence);

  for (var i = 0; i < this.editingContexts.length; i++) {
    var c = this.editingContexts[i];
    if (c != context && c.onPresence) c.onPresence(this.presence);
  }
};

Doc.prototype.pOpApply = function(pOp) {
  if (!this.type) return;
  for (var i = 0; i < this.pendingData.length; i++) {
    _transformPresence(this.type, pOp, this.pendingData[i]);
  }
  var p = {data:this.presence};
  var err = _applyPresence(p, pOp);
  if (err) return console.error(err);
  this.presence = p.data;
  this.emit('presence op', pOp);
  this._onPresenceUpdated();
};
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  var selection = this.myPresence._selection;
  if (opData.op && selection != null && this.type.transformSelection) {
    this.myPresence._selection = this.type.transformSelection(selection, opData.op, true);
  }
  this.myPresenceDirty = false;
...
Doc.prototype.submitPOp = function(pOp) {
  throw Error('Not implemented');
};

Doc.prototype.setPresenceProperty = function(key, value) {
  this.myPresence[key] = value;
  this.myPresenceDirty = true;

  var self = this;
  setTimeout((function() { self.flush(); }), 0);
}

Doc.prototype.setSelection = function(selection) {
  // This should use submitPOp, above.
  var mySelection = this.myPresence._selection;
  if (selection === mySelection
      || (mySelection != null && this.type && this.type.selectionEq
          && this.type.selectionEq(mySelection, selection)))
    return;

  this.setPresenceProperty('_selection', selection);
};


Doc.prototype.broadcast = function(msg) {
  // Not entirely sure how to implement this - in particular, should it be tied
  // in to the regular flush() cycle (ie, is it allowed to be concurrent with
  // subscriptions?
  throw Error('broadcast not implemented');
};
...
Doc.prototype.createContext = function() {
  ...
  var context = {
    ...
    setPresenceProperty: function(key, value) {
      doc.setPresenceProperty(key, value);
    },
  ...
  if (context._register) context._register();
...
function _transformPresence(type, p, pOp, opData) {
  if (!type) return;
  if (!type.transformSelection) return;

  // This is not complete. .... ........
  if (pOp.p && pOp.p.length === 2 && pOp.p[1] === '_selection' && opData.op) {
    // Gasp...
    pOp.val = type.transformSelection(pOp.val, opData.op, pOp.p[0] === opData.src);
  }
};

// Apply a normal op to the presence data. Probably a bad name.
function _updatePresence(type, p, opData) {
  if (!p) return;

  if (opData.op != null && !type.transformSelection) return;
  ...
  for (var id in p) {
    ...
    if (d != null && d._selection != null) {
      if (opData.op) {
        d._selection = type.transformSelection(d._selection, opData.op, id === opData.src);
      } else if (opData.del) {
        delete d._selection;
      }
    }
